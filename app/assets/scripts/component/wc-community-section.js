...
class theCommunitySection extends HTMLElement {
  constructor() {
    ...
    this.trendButtonElement.addEventListener(
      "click",
      this.filterTrend.bind(this)
    );
  ...
  // render
  #render(community) {
    document.getElementById("community").insertAdjacentHTML(
      "afterbegin",
      `
            <h1 class="community__name">>> ${community.communityName}</h1>`
    );
    document.getElementById("community-detail").insertAdjacentHTML(
      "afterbegin",
      `
    ...
    postElement.insertAdjacentHTML(
      ...
      `
            ...
            <div class="post__reactions">
                <agree-disagree agreeCount=${post.agreeCount} disagreeCount=${
        post.disagreeCount
      } isAgreeClicked=${false} isDisAgreeClicked=${false}></agree-disagree>
